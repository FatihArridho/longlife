/* ===============================
   script.js — Full (fixed start/cinematic bug)
   - semua fitur sebelumnya (scene, photobooth, music via mp3, effects, TV, pet, konami, hearts, balloons, birds, sounds)
   - perbaikan: start button & cinematic done handler (tidak memakai window.event)
   =============================== */
...
function resizeCanvas(){
  ...
  if (effectCanvas) { effectCanvas.width = 800; effectCanvas.height = 600; }
...
/* balloons */
const balloons = [];
class Balloon{ constructor(x){ this.x = x ?? rand(40,760); this.y = 620 + rand(0,80); this.vy = rand(-0.6, -1.6); this.vx = rand(-0.2,0.2); this.color = `hsl(${rand(0,360)},80%,60%)`; this.w = rand(10,18); this.age=0; } update(){ this.x += this.vx; this.y += this.vy; this.age++; if(this.y < -60) this.reset(); } draw(g){ g.fillStyle = this.color; g.beginPath(); g.ellipse(this.x, this.y, this.w, this.w*1.1, 0, 0, Math.PI*2); g.fill(); g.strokeStyle='rgba(255,255,255,0.3)'; g.beginPath(); g.moveTo(this.x, this.y+this.w*1.1); g.lineTo(this.x, this.y + this.w*3); g.stroke(); } reset(){ this.x = rand(40,760); this.y = 620 + rand(0,80); this.vy = rand(-0.6,-1.6); this.vx = rand(-0.2,0.2); this.color = `hsl(${rand(0,360)},80%,60%)`; } }
function spawnBalloon(){ balloons.push(new Balloon()); }
setInterval(()=>{ if(Math.random()<0.6) spawnBalloon(); },1300);

/* hearts (hover) */
const hearts = [];
class Heart{ constructor(x,y){ this.x=x; this.y=y; this.vx = rand(-0.8,0.8); this.vy = rand(-2.4,-1.2); this.life=80; this.age=0; this.col = `hsl(${rand(320,350)},80%,70%)`; } update(){ this.x+=this.vx; this.y+=this.vy; this.vy += 0.04; this.age++; } draw(g){ g.save(); const a = 1 - (this.age/this.life); g.globalAlpha = a; g.fillStyle = this.col; g.fillRect(this.x, this.y, 4,4); g.fillRect(this.x+4, this.y, 4,4); g.fillRect(this.x+2, this.y+3, 4,4); g.restore(); } }
function emitHeartsAt(x,y,count=6){ for(let i=0;i<count;i++) hearts.push(new Heart(x + rand(-6,6), y + rand(-6,6))); }

/* birds */
const birds = [];
class Bird{ constructor(){ this.y = rand(40,120); this.x = rand(-100,-20); this.vx = rand(1.2,3.2); this.frame = 0; this.timer=0; this.color = `hsl(${rand(0,360)},80%,60%)`; } update(){ this.x += this.vx; this.timer++; if(this.timer>8){ this.timer=0; this.frame=(this.frame+1)%2; } } draw(g){ g.fillStyle=this.color; const px = 2; g.fillRect(this.x, this.y, 6, 2); if(this.frame===0){ g.fillRect(this.x+2, this.y-2, 2,2); } else { g.fillRect(this.x+2, this.y+2, 2,2); } } }
...
for(const k in SOUND) { try{ SOUND[k].volume = 0.22; SOUND[k].preload = 'auto'; }catch(e){} }
function playSound(name){ const s = SOUND[name]; if(!s) return; try{ s.currentTime = 0; s.play().catch(()=>{}); }catch(e){} }

/* ---------- MUSIC (mp3 url) ---------- */
const MUSIC_URL = 'https://k.top4top.io/m_3628wytop1.mp3';
...
/* ---------- PHOTOBOOTH (simplified reuse) ---------- */
let mediaStream = null; let facingMode = 'user'; let pixelateEnabled = false;
if (cameraEffectCanvas) { cameraEffectCanvas.width = 800; cameraEffectCanvas.height = 600; }
const camECTX = cameraEffectCanvas ? cameraEffectCanvas.getContext('2d') : null;
if (camECTX) camECTX.imageSmoothingEnabled=false;
...
function ensurePhotoUI_simple(){
  const card = photoboothModal ? photoboothModal.querySelector('.pixel-card') : null;
  if(!card) return;
  ...
  const actions = card.querySelector('.modal-actions'); if(actions) card.insertBefore(extra, actions); else card.appendChild(extra);
}
ensurePhotoUI_simple();

async function startCamera(){ try{ ensurePhotoUI_simple(); stopCamera(); const constraints = { video: { facingMode: { ideal: facingMode }, width: { ideal:1280 }, height: { ideal:720 } }, audio:false }; mediaStream = await navigator.mediaDevices.getUserMedia(constraints); cameraVideo.srcObject = mediaStream; await cameraVideo.play(); requestAnimationFrame(drawCameraPreviewAndDetect); } catch(err){ console.warn('camera error', err); alert('Kamera tidak bisa diakses.'); stopCamera(); if(photoboothModal) photoboothModal.classList.add('hidden'); } }
async function restartCamera(){ stopCamera(); await new Promise(r=>setTimeout(r,180)); await startCamera(); }
function stopCamera(){ if(mediaStream){ mediaStream.getTracks().forEach(t=>t.stop()); mediaStream=null; } try{ cameraVideo.pause(); cameraVideo.srcObject=null; }catch(e){} if(camECTX) camECTX.clearRect(0,0,800,600); }

let lastDetectionTime=0;
async function drawCameraPreviewAndDetect(ts){ if(!camECTX) return; camECTX.clearRect(0,0,800,600); camECTX.fillStyle='rgba(0,0,0,0.06)'; camECTX.fillRect(0,0,800,600); const vw=cameraVideo.videoWidth, vh=cameraVideo.videoHeight; if(vw && vh){ const scale = Math.max(800/vw,600/vh); const sw = 800/scale, sh=600/scale, sx=(vw-sw)/2, sy=(vh-sh)/2; camECTX.drawImage(cameraVideo, sx, sy, sw, sh, 0,0,800,600); } if(!photoboothModal || !photoboothModal.classList.contains('hidden')) requestAnimationFrame(drawCameraPreviewAndDetect); else camECTX.clearRect(0,0,800,600); }
...
async function captureAndDownload(){
  ...
  if(camECTX){ camECTX.fillStyle='rgba(255,255,255,0.06)'; camECTX.fillRect(0,0,800,600); setTimeout(()=>camECTX.clearRect(0,0,800,600),40); }
}
...
function initTV(){
  tv.channels = [
    (g)=>{ for(let i=0;i<tv.w;i+=4) for(let j=0;j<tv.h;j+=4){ g.fillStyle = Math.random() < 0.5 ? '#fff':'#000'; g.fillRect(tv.x+i,tv.y+j,4,4); } },
    (g)=>{ g.fillStyle='#000'; g.fillRect(tv.x,tv.y,tv.w,tv.h); g.fillStyle='#fff'; g.fillRect(tv.x+22,tv.y+12,12,6); g.fillRect(tv.x+18,tv.y+6,2,2); g.fillRect(tv.x+32,tv.y+6,2,2); g.fillRect(tv.x+20,tv.y+18,16,6); },
    (g)=>{ g.fillStyle='#000'; g.fillRect(tv.x,tv.y,tv.w,tv.h); g.fillStyle='#ff0'; g.font='10px monospace'; g.fillText('HBD!', tv.x+12, tv.y+36); },
    (g)=>{ for(let i=0;i<tv.w;i+=8){ g.fillStyle=`hsl(${(i/2)+tv.timer*4},80%,60%)`; g.fillRect(tv.x+i,tv.y+Math.sin((i+tv.timer)/10)*6 + 20,6,10); } }
  ];
}
initTV();
function drawTV(g){
...
function wireHoverHearts(){
  ...
  buttons.forEach(b=>{
    ...
    b.addEventListener('mouseenter',(ev)=>{ const r=b.getBoundingClientRect(); const cx = r.left + r.width/2; const cy = r.top + r.height/2; const cr = canvasWrap.getBoundingClientRect(); const scaleX = canvas.width / cr.width; const scaleY = canvas.height / cr.height; const tx = (cx - cr.left) * scaleX; const ty = (cy - cr.top) * scaleY; emitHeartsAt(tx,ty,8); });
  });
}
...
function drawScene(){
  ...
  try { applyHeatWarp(ctx, ctx, warpStrength, {x:360,y:240,w:96,h:60}); } catch(e) {}
...
function playCinematicThenStart(){
  ...
  Object.assign(overlay.style,{position:'fixed',left:0,top:0,width:'100%',height:'100%',background:'#000',color:'#fff',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',fontFamily:'monospace'});
  document.body.appendChild(overlay);

  const lines = ['loading scene...','booting birthday engine...','press any key to continue'];
  let idx = 0;
  const txt = document.createElement('div'); txt.style.fontSize='20px'; txt.style.letterSpacing='2px'; txt.textContent = lines[idx];
  ...
  const step = () => {
    ...
    if (idx < lines.length) {
      ...
      txt.textContent = 'press any key or click to continue';
      const done = () => {
        window.removeEventListener('keydown', done);
        overlay.remove();
        cinematicPlaying = false;
        startScreen && startScreen.classList.add('hidden');
        mainScene && mainScene.classList.remove('hidden');
        drawScene();
        drawEffects();
        playSound('coin');
      };
...
/* ---------- UI HOOKS (start fixed, no window.event) ---------- */
startBtn?.addEventListener('click', () => {
  playSound('click');
  playCinematicThenStart();
});
startBtn?.addEventListener('keydown', e => {
  if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); startBtn.click(); }
});

openLetterBtn?.addEventListener('click', ()=>{ letterModal && letterModal.classList.remove('hidden'); playSound('click'); });
closeLetterBtn?.addEventListener('click', ()=>{ letterModal && letterModal.classList.add('hidden'); playSound('click'); });
kirimBtn?.addEventListener('click', ()=>{ const msg=birthdayMessage?.value.trim(); if(!msg){ alert('Pesan kosong'); return; } window.location.href = `https://wa.me/6281511118515?text=${encodeURIComponent(msg)}`; spawnConfettiBurst(400,120); for(let i=0;i<3;i++) spawnFirework(300+i*80 + rand(-30,30)); letterModal && letterModal.classList.add('hidden'); downloadBtn && downloadBtn.classList.remove('hidden'); playSound('coin'); });

crtToggle?.addEventListener('click', ()=>{ isCrtOn = !isCrtOn; document.body.classList.toggle('crt-on', isCrtOn); playSound('click'); });

photoBoothBtn?.addEventListener('click', async ()=>{ photoboothModal && photoboothModal.classList.remove('hidden'); ensurePhotoUI_simple(); await startCamera(); playSound('click'); });
closePhotoBtn?.addEventListener('click', ()=>{ photoboothModal && photoboothModal.classList.add('hidden'); stopCamera(); playSound('click'); });
takePhotoBtn?.addEventListener('click', async ()=>{ takePhotoBtn.disabled=true; await captureAndDownload(); setTimeout(()=>{ photoboothModal && photoboothModal.classList.add('hidden'); stopCamera(); takePhotoBtn.disabled=false; },700); playSound('snap'); });
